...
**算法**：贪心初始（按轮次顺序、时段按 global_prime_score 降序、硬约束过滤）+ 模拟退火（邻域=换馆/换时段/两场交换，目标=Z₂−50×违反数驱动可行化），8 起点。

**最终 Z₂ = 19.1088**（warm-start SA 优化，从 19.0164 提升至 19.1088）。
...
**多起点稳定性**：8 起点 SA + warm-start 精修后 Z₂ = 19.1088，约束全部 0 违反。
...
**Z₃**：动态 = 7.9430，静态 = 6.6647，**动态改善 19.18%**（动态≥静态，符合递进性）。
...
- **P2**：8 起点 Z₂ 标准差约 0.04，稳定；权重 ±20% 时 Z₂ 变化 <5%（objective_weights 灵敏度低）。
...
python3 code/problem2.py           # P2 优化（约 5-8 分钟，8 起点 SA）
