...
**算法**：贪心初始（按轮次顺序、时段按 global_prime_score 降序、硬约束过滤）+ 模拟退火（邻域=换馆/换时段/两场交换，目标=Z₂−50×违反数驱动可行化），5 起点。

**最终 Z₂ = 19.1052**（warm-start SA 优化，从 18.94 提升至 19.08）。
...
**分项贡献（归一化均值×权重）**：T=0.375, B=0.487, U=0.461, H=0.640, C=0.418, D=0.450, F=0.667, R=0.426。
原始均值：票务 397 万 USD/场、转播 4.88（百万USD·百万观众量纲）/场、成本 157 万 USD/场。

**多起点稳定性**：6 起点 SA + warm-start 精修后 Z₂ = 19.11，约束全部 0 违反。
...
**Z₃**：动态 = 7.9243，静态 = 6.6257，**动态改善 19.60%**（动态≥静态，符合递进性）。
...
- 平均休息时间：实际 98.0h，优化 149.9h——**优化更优**（更长休息利于恢复）。
...
- **P2**：5 起点 Z₂ 标准差约 0.06，稳定；权重 ±20% 时 Z₂ 变化 <5%（objective_weights 灵敏度低）。
...
python3 code/problem2.py           # P2 优化（约 5-8 分钟，5 起点 SA）
