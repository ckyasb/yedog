...
| **P4 策略优化** | 兼顾充电时间与寿命的多目标优化 | 解析充电时间模型($R^2$=0.884) + log 空间衰减模型($R^2$=0.805) + Pareto 前沿 | $R^2_{\text{log}}=0.805$ |
...
3. **推荐快充策略**：$C_1 = 5.52,\ Q_1 = 78,\ C_2 = 5.78$，充电时间 8.91 min，模型预测寿命 13 652 次。该策略将 SOC 切换点前移至 $Q_1 = 78\%$，使中高 SOC 段暴露容量仅 2%、$E_{\text{high}} \approx 11.6$ 可控，在充电效率与寿命间取得平衡。
...
| P4 充电时间模型 $R^2$ | 0.884 | `results/p4_charge_time_calib.csv` |
| P2 SOC 衰减模型 $R^2$ | 0.313 (log 非线性) | `results/p2_soc_interval.json` |
| **推荐策略** | $C_1=5.52,\ Q_1=78,\ C_2=5.78$ | `results/p4_bayes_optimized.json` |
| 推荐寿命 / 充电时间 | 13 652 次 / 8.91 min | 同上 |
