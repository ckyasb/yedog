...
决策空间约 $9.2 times 10^4$ 组合，精确整数规划成本高，采用贪心初始 + 模拟退火 + 多起点启发式。贪心按轮次顺序处理，并为每轮设定日期窗口（第一轮 06-11 至 06-14、第二轮 06-14 至 06-17、第三轮 06-17 至 06-30）以保证 60 小时休息可行并为后续轮留容量；候选时段按日期升序与黄金时段评分排序，硬约束过滤后按近似目标评分选取。模拟退火邻域为换馆、换时段与两场交换，目标为 $Z_2 - 50 times "违反数"$ 驱动可行化，8 起点并行后对剩余 60 小时违反做定向修复。求解流程如图 9 所示。
...
贪心初始 + 模拟退火 + 8 起点并行 + 定向修复 + warm-start SA 精修后，最终 $Z_2 = 19.1088$。9 类约束回代全部为 0 违反，其中第三轮每日高等级安保容量逐日校验均满足（如 06-23 为 3 场、上限 3；06-24 为 2 场、上限 2；06-29 为 2 场、上限 2）。各指标归一化均值与对 $Z_2$ 的贡献如表 2 与图 10 所示。
...
场馆与日期的使用分布如图 11 所示，72 场均匀分布于 16 个场馆与 20 天中，单日单馆承办不超过其上限。多起点稳定性如图 12 所示，8 起点 $Z_2$ 标准差约 0.04，高度稳定。多目标权衡如图 13 雷达图所示，优化方案在收益类指标上接近理想，成本与风险类处于中等水平。
