...
贪心初始 + 模拟退火 + 6 起点并行 + 定向修复 + warm-start SA 精修后，最终 $Z_2 = 19.0832$。9 类约束回代全部为 0 违反，其中第三轮每日高等级安保容量逐日校验均满足（如 06-23 为 3 场、上限 3；06-24 为 2 场、上限 2；06-29 为 2 场、上限 2）。各指标归一化均值与对 $Z_2$ 的贡献如表 2 与图 10 所示。

#align(center)[#text(font: ("Heiti SC","STHeiti","SimSun","Songti SC"), size: 12pt)[表 2 #h(1em) 问题二各指标归一化均值与贡献]]
#align(center)[
  #table(
    ...
    [转播 $B$], [0.487], [+0.25], [+0.122],
    [不确定 $U$], [0.461], [+0.15], [+0.069],
    [吸引 $H$], [0.640], [+0.10], [+0.064],
    [成本 $C$], [0.418], [-0.08], [-0.033],
    [旅行 $D$], [0.450], [-0.07], [-0.031],
    [公平 $F$], [0.667], [-0.06], [-0.040],
    [风险 $R$], [0.426], [-0.04], [-0.017],
    table.hline(stroke: 0.8pt),
  )
]
