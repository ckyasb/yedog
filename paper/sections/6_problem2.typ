...
贪心初始 + 模拟退火 + 6 起点并行 + 定向修复 + warm-start SA 精修后，最终 $Z_2 = 19.1185$。9 类约束回代全部为 0 违反，其中第三轮每日高等级安保容量逐日校验均满足（如 06-23 为 3 场、上限 3；06-24 为 2 场、上限 2；06-29 为 2 场、上限 2）。各指标归一化均值与对 $Z_2$ 的贡献如表 2 与图 10 所示。
