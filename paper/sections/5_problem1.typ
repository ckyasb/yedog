...
同时建立三个模型：梯度提升回归（HistGradientBoosting，捕捉非线性与特征交互）、岭回归（Ridge，线性、泛化稳定）与 Lasso 回归（L1 正则化、特征选择）。最终预测取三模型混合（0.4 Lasso + 0.3 HGB + 0.3 Ridge），以在拟合度与泛化间取得平衡。模型评价采用 5 折 GroupKFold，按比赛年月分组，避免时序泄露。求解流程如图 3 所示。
