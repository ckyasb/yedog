...
#abstract-cn[
  ...
  针对问题二，建立 $Z_2 = 0.25T+0.25B+0.15U+0.10H-0.08C-0.07D-0.06F-0.04R$ 的综合目标函数，决策变量为 72 场的场馆与开球时段。采用贪心初始（按轮次划分日期窗口保证 60 小时休息可行）+ 模拟退火 + 6 起点并行 + 定向修复 + warm-start SA 精修。最终 $Z_2 = 19.08$，9 类约束全部 0 违反。

  针对问题三，在第三轮 24 场固定赛程上，严格按题面 14 式公式链更新球队竞技状态 $S_t$、进球均值 $lambda_i$、晋级概率 $p_t$（蒙特卡洛 20000 次，向量化实现）、更新吸引力 $A_i$、现场/转播需求与各类风险，再以分场枚举离散决策 + $delta$ 一维优化 + 每日容量耦合调整求解 $Z_3$。动态方案 $Z_3 = 7.92$，静态方案 $Z_3 = 6.64$，动态改善 19.27%，全部场次动态净效益不低于静态。

  针对问题四，采集 2022 卡塔尔世界杯 48 场小组赛实际赛程，采用与问题二相同的指标定义与标准化方法，对规模差异用人均/比例口径处理。对比显示优化方案在平均休息时间（147.6h vs 98.0h）、黄金时段覆盖率（76.4% vs 33.3%）上显著占优，实际方案因单时区在跨时区负担上占优。]
][
