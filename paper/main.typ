...
#abstract-cn[
  ...
  对于问题三（4000 条带时间窗与任务类型的多日排班，24 架飞机配额），采用两阶段策略：阶段 A 排非临时 3840 条得总飞机使用时间 $T_0$=53358 min，阶段 B 将 160 条临时任务以零额外时间插入现有架次，阶段 C 空位补客将未排需求塞入已有架次空位，阶段 D 机型换型将 10 个 T3 架次降为 T2，阶段 E 停靠序约束 TSP 重排 24 个架次。最终总飞机使用时间 53096 min，临时任务满足 160/160（100%），非临时服务 3713/3840（96.7%）。所有结果均经独立校验，续航、时刻链、运营窗与周转约束全部满足。
][
